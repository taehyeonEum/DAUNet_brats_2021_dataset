...
def parse_args_(args):
    ...
    parser.add_argument(
        '--input_channel', type=int, default=3, required=True,
        help = 'enter channels of input image'
    )

    parser.add_argument(
        '--model_name', type=str, default = None, required=False,
        help = "if you already have model and just want to visualize result enter model's name. Also you have to select visualization type."
    )
...
VALID_IMAGE_PATH = os.path.join(DATA_PATH, 'Valid_img')
VALID_MASK_PATH  = os.path.join(DATA_PATH, 'Valid_msk')

# SHOULD be maken befor run train.py
OUTPUT_PATH = '/home/NAS_mount/thum/brats/output'
# OUTPUT_PATH = './output'
...
MODEL_NAME = 'Brats_DAUNetV4_9_aug2'
OPTIMIZER = 'Adam'
