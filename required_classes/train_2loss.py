...
from required_classes.loss_functions.customLoss_dice_bce4 import customLoss as customLoss4
...
class Train_DBC:
    ...
    def run(self):
        ...
        def xcel(start_row, wb, ws):

            columns = ['Epoch', 'train_cost', 'train_bce_cost', 'train_dsc_cost', 'train_fp_cost', 'train_fn_cost', 'test_zero_dsc',  'test_nonzero_dsc']
        ...
        def train_model(_model, _epoch, _train_dataset, _train_dataloader, _optimizer, _lr_sche, _creterion, _array_cost, _array_accu_zero, _array_accu_nonzero, _array_cost_bce, _array_cost_bce2, _array_cost_dsc, _log_path):
            ...
            for batch_idx, data in enumerate(_train_dataloader):
                ...
                cost_bce, cost_dsc = _creterion(prediction, y, cf.POS_WEIGHT, n_args['device'], cf.SMOOTH)
                
                bce_cost_sum += cost_bce.item() * cf.BCE_WEIGHT
                dsc_cost_sum += cost_dsc.item() * cf.DSC_WEIGHT

                cost = cost_bce*torch.Tensor([cf.BCE_WEIGHT]).to(n_args['device']) + cost_dsc*torch.Tensor([cf.DSC_WEIGHT]).to(n_args['device'])
                cost.backward()
                _optimizer.step()
                _lr_sche.step()
                ...
                if (batch_idx + 1) % (20) == 0:
                    ...
                    log = 'Epoch:{:2d}/{} batch_idx:{:3d} Train Cost Total:{:2.6f} BCE:{:2.6f} DSC:{:2.6f}'.format(_epoch, cf.NUM_EPOCHS, batch_idx+1, train_cost_avg, train_cost_bce_avg, train_cost_dsc_avg)
                    print(log)
                    if n_args['log_param']:    
                        f = open(_log_path, 'a')
                        f.write(log + '\n')
        ...
        def test_model(_model, _test_dataset, _test_dataloader, _creterion, _array_cost, _array_accu_zero, _array_accu_nonzero, _array_cost_bce, _array_cost_bce2, _array_cost_dsc, _log_path):
            ...
            with torch.no_grad():
                for batch_idx, data in enumerate(_test_dataloader):
                    ...
                    test_cost_sum += ( cost_bce.item() * cf.BCE_WEIGHT + cost_dsc.item() * cf.DSC_WEIGHT )
            ...
            zero_accuracy = (zero_dice_score_sum/ zero_count_sum)    
            nonzero_accuracy = (dice_score_sum / nonzero_count_sum)
            log = 'Test Cost ToTal: {:2.6f} BCE: {:2.6f} DSC {:2.6f} Test Accuracy:zero{:2.6f}_nonzero:{:2.6f}'.format(test_cost_avg, test_cost_bce_avg, test_cost_dsc_avg, zero_accuracy, nonzero_accuracy)
            print(log)
            if n_args['log_param']:    
                f = open(_log_path, 'a')
