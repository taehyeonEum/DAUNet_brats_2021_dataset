...
from models.DAUNetV4 import DAUNet
...
class Train_DBC:
    ...
    def run(self):
        ...
        model = DAUNet(n_args['input_channel'], cf.STARTING_CHANNEL, cf.ENCODING_CHANNELS, cf.DECODING_CHANNELS, cf.GROWTH_RATE).to(n_args['device'])
        ...
        def save_xl(start_parameter):
            columns = ['Epoch', 'train_cost', 'train_bce_cost', 'train_dsc_cost', 'test_zero_dsc',  'test_nonzero_dsc', 'test_total_dsc']
            ...
            for i in range(len(x_array)):
                ...
                ws.cell(row=i+start_row, column=13).value = test_total_dsc_arr[i]
        ...
        # %%
        def train_model(_model, _epoch, _train_dataset, _train_dataloader, _optimizer, _lr_sche, _creterion, _array_cost, _array_accu_zero, _array_accu_nonzero, _array_cost_bce, _array_cost_bce2, _array_cost_dsc, _array_accu_total, _log_path):
            now = time.localtime()
            ...
            bce2_cost_sum = 0
            dsc_cost_sum = 0
            tumor_dice_score_sum = 0
            zero_dice_score_sum = 0
            total_dice_score_sum = 0
            zero_count_sum = 0
            nonzero_count_sum = 0
            print('----------------train----------------')
            for batch_idx, data in enumerate(_train_dataloader):
                ...
                zero_dsc, nonzero_dsc, total_dsc, zero_count, nonzero_count = dice_score(prediction, y)
                zero_dice_score_sum += zero_dsc
                tumor_dice_score_sum += nonzero_dsc
                total_dice_score_sum += total_dsc
            ...
            total_accuracy = (total_dice_score_sum / (nonzero_count_sum + zero_count_sum))
            nonzero_accuracy = (tumor_dice_score_sum / nonzero_count_sum)
            zero_accuracy = (zero_dice_score_sum/ zero_count_sum)
            log = 'Epoch:{:2d}/{} batch_idx:{:3d} Train Accuracy:zero{:2.6f}_nonzero:{:2.6f}_total:{:2.6f}'.format(_epoch, cf.NUM_EPOCHS, batch_idx, zero_accuracy, nonzero_accuracy, total_accuracy)
            print(log)
            if n_args['log_param']:    
                f = open(_log_path, 'a')
                f.write('\nTrain Accuracy:\n    '+ log + '\n')
                ...
                _array_accu_total.append(total_accuracy)

        # %%
        def test_model(_model, _test_dataset, _test_dataloader, _creterion, _array_cost, _array_accu_zero, _array_accu_nonzero, _array_cost_bce, _array_cost_bce2, _array_cost_dsc, _array_accu_total, _log_path):
            test_cost_sum = 0
            bce_cost_sum = 0
            bce2_cost_sum = 0
            dsc_cost_sum = 0
            ...
            total_accuracy = (total_dice_score_sum/ (zero_count_sum + nonzero_count_sum))    
            zero_accuracy = (zero_dice_score_sum/ zero_count_sum)    
            nonzero_accuracy = (tumor_dice_score_sum / nonzero_count_sum)
            log = 'Test Cost ToTal: {:2.6f} BCE: {:2.6f} NEW_LOSS: {:2.6f} DSC {:2.6f} Test Accuracy:zero{:2.6f}_nonzero:{:2.6f}_total:{:2.6f}'.format(test_cost_avg, test_cost_bce_avg, test_cost_bce2_avg, test_cost_dsc_avg, zero_accuracy, nonzero_accuracy, total_accuracy)
            print(log)
            if n_args['log_param']:    
                f = open(_log_path, 'a')
        ...
        def test_model_total_score(_testloader, model_name, _log_path):
            ...
            with torch.no_grad():
                for batch_idx, data in enumerate(_testloader):
                    ...
                    classification_sc, tumor_dsc_sc, total_dsc_sc, iou_sc, pixaccu_sc, precision_sc, recall_sc, hd_sc, zero_c, nonzero_c = total_score(pred, y)
                    classification_sum += classification_sc
                    tumor_dsc_sum += tumor_dsc_sc
                    total_dsc_sum += total_dsc_sc
                    iou_sum += iou_sc
                    ...
                    if (batch_idx+1)%10 == 0:
                        ...
                        info_list = [('zero_slice_classification', classification_sum/zero_count),
                                     ('tumor_dsc',tumor_dsc_sum/(nonzero_count)), 
                                     ('total_dsc',total_dsc_sum/(nonzero_count + zero_count)), 
                                     ('iou',iou_sum/(nonzero_count + zero_count)),
        ...
        for epoch in range(cf.NUM_EPOCHS):
            ...
            model.train()
            train_model(model, epoch+1, trainset, trainloader, optimizer, lr_sche, creterion, train_cost_arr, train_zero_dsc_arr, train_nonzero_dsc_arr, train_cost_bce_arr, train_cost_bce2_arr, train_cost_dsc_arr, train_total_dsc_arr, log_path)

            model.eval()
            test_model(model, testset, testloader, creterion, test_cost_arr, test_zero_dsc_arr, test_nonzero_dsc_arr, test_cost_bce_arr, test_cost_bce2_arr, test_cost_dsc_arr, test_total_dsc_arr, log_path)
            ...
            plt.subplot(1,3,2)
            plt.plot(x_array, train_zero_dsc_arr, 'r')
            plt.plot(x_array, train_nonzero_dsc_arr, 'g')
            plt.plot(x_array, train_total_dsc_arr, 'b')
            plt.xlabel('EPOCH')
            plt.ylabel('ZERO ACCURACY')
            plt.title('{}Train Normal(R)/Tumor(G)/Total(B) slices F1-score{}'.format(model_name, epoch))
            
            plt.subplot(1,3,3)
            plt.plot(x_array, test_zero_dsc_arr, 'r')
            plt.plot(x_array, test_nonzero_dsc_arr, 'g')
            plt.plot(x_array, test_total_dsc_arr, 'b')
            plt.xlabel('EPOCH')
            plt.ylabel('NONZERO ACCURACY')
            plt.title('Test Normal(R)/Tumor(G)/Total(B) slices F1-score')
            
            if n_args['log_param']:
                plt.savefig(os.path.join(cf.GRAPH_PATH, model_name) + '.png' ,facecolor='#ffffff',edgecolor='blue')
                plt.savefig(os.path.join('./_graphs', model_name) + '.png' ,facecolor='#ffffff',edgecolor='blue')
            plt.show()

            if test_nonzero_dsc_arr[-1] > 0.4 and max(test_nonzero_dsc_arr) == test_nonzero_dsc_arr[-1]:
                torch.save(model, model_path + '/{}_ep:{}_tumor{:1.4f}_total{:1.4f}.pth'.format(model_name, epoch ,max(test_nonzero_dsc_arr), test_total_dsc_arr[-1]))
                print(model_path + '_{}_{:1.3f}.pth'.format(epoch ,max(test_nonzero_dsc_arr)))
