...
def total_score(pred, gt):
    ...
    for i in range(len(pred)):
        ...
        iou_sum += (tp + cf.SMOOTH) / (tp + fn + fp + cf.SMOOTH)
        total_dsc_sum += (2*tp + cf.SMOOTH) / (2*tp + fn + fp + cf.SMOOTH)

    nonzero_count = len(pred)-zero_count
    return (zero_slice_classification_sum, tumor_dsc_sum, total_dsc_sum, iou_sum, pixaccu_sum, precision_sum, recall_sum, hd_sum, zero_count, nonzero_count)
