...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, growth_rate):
        ...
        self.aftcat = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(inner_channel * 4, growth_rate, kernel_size=3, padding=1, bias=False),
        )
...
class UpConvs(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels*2, out_channels*2, kernel_size=5, padding=2, bias=False),
        )
        
        self.conv2 = nn.Sequential(
            nn.BatchNorm2d(out_channels*2),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels*2, out_channels, kernel_size=5, padding=2, bias=False),
        )
...
class Encoder(nn.Module):
    def __init__(self, input_channel, starting_channel, channels, growth_rate):
        super().__init__()
        self.channels = channels
        self.growth_rate = growth_rate
        self.starting_channel = starting_channel
        self.convInit = nn.Conv2d(input_channel, starting_channel, 3, padding=1)
...
class Decoder(nn.Module):
    def __init__(self, channels): #channels decrease by 1/2
        ...
        self.upconvs = nn.ModuleList([
            UpConvs(channels[i], channels[i+1]) for i in range(len(channels)-1)
        ])
...
class DAUNet(nn.Module):
    def __init__(self, input_channel, starting_channel, encChannels, decChannels, growth_rate):
        super().__init__()        
        self.encoder = Encoder(input_channel, starting_channel, encChannels, growth_rate)
